...
# len(list) - 리스트의 길이 반환
# range(시작:끝:간격) - 함수는 반복 가능한 객체를 생성
# 리스트.append(요소) - 리스트에 요소를 추가 ex([] <= [0, 0, 0] 3번 넣으면 [ [0, 0, 0], [0, 0, 0], [0, 0, 0] ] 3x3 배열 생성)

# n*n 배열을 오른쪽으로 90도 회전시키는 방법 - 새로운 배열을 만들어서 회전된 위치에 값을 채워넣기 [j][n-1-i], 왼쪽으로 90도 회전시키는 방법 - [n-1-i][j]     